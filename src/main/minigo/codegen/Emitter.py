...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        if type(typ) is IntLiteral or type(typ) is IntType:
            return self.emitPUSHICONST(in_, frame)
        elif type(typ) is FloatLiteral or type(typ) is FloatType:
            return self.emitPUSHFCONST(in_, frame)
        elif type(typ) is BooleanLiteral or type(typ) is BoolType:
            if in_ == True : return self.emitPUSHICONST(1, frame)
            else : return self.emitPUSHICONST(0, frame)
        elif type(typ) is StringLiteral or type(typ) is StringType:
            frame.push()
    ...
    *
    ...
    def emitREADVAR2(self, name, typ, frame):
        ...
        elif type(typ) is cgen.ArrayType or type(typ) is cgen.ClassType or type(typ) is StringType:
            return self.jvm.emitAALOAD()
        else:
            raise IllegalOperandException(name)
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        if frame.currOpStackSize > 0: frame.pop()
    ...
    *
    ...
    def emitMULTIANEWARRAY(self, dim, typeIn, lexeme, frame) :
        frame.push()
        if (type(typeIn) is IntType or type(typeIn) is FloatType or type(typeIn) is BoolType) and dim == 1:
            return self.jvm.emitNEWARRAY(lexeme)
        elif type(typeIn) is StringType and dim == 1:
            return self.jvm.emitANEWARRAY(lexeme)
        else :
            return self.jvm.emitMULTIANEWARRAY(lexeme, dim)
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitFIELD(self, lexeme, typ, isFinal, value=None):
        typ = self.getJVMType(typ)
        if isFinal:
            return "\n.field public final " + lexeme + " " + typ + ((" = " + value) if value else "") + JasminCode.END
        else:
            return "\n.field public " + lexeme + " " + typ + ((" = " + value) if value else "") + JasminCode.END
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESTATIC(self, lexeme, in_, frame):
        ...
        if (frame.currOpStackSize > 0) : list(map(lambda x: frame.pop(), typ.partype))
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        elif type(in_) is StringType and op == "+" :
            result.append(self.jvm.emitINVOKEVIRTUAL("java/lang/String/", "concat(Ljava/lang/String;)Ljava/lang/String;"))
            return ''.join(result)
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def emitMETHOD(self, lexeme, in_, isStatic, frame):
        ...
        if type(frame) is bool :
            if frame == True :  return self.jvm.emitMETHOD(lexeme, self.getJVMType(in_), isStatic, True)    
    ...
    def emitENDMETHOD(self, frame):
        ...
        if type(frame) is bool :
            if frame == True : return ''.join(self.jvm.emitENDMETHOD())
    ...
    *   .super java/lang/Object<p>
    ...
    def emitPROLOG(self, name, parent):
        ...
        if temp[-1] != '!' : result.append(self.jvm.emitCLASS("public " + name))
        else :
